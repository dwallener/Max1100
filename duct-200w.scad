...
part = "assembly"; // assembly, duct, collector, transition, outlet, stand, adapter, lid, interface
show_cards = true;
wall = 3;
...
if(part=="interface") {
    ...
    color([0.8,0.8,0.82]) lid(65);
    for(i=[0:3]) color([0.3,0.45,0.6,0.4])
        translate([(i-1.5)*card_pitch-card_w/2,-90,0]) cube([card_w,90,card_h]);
}
...
if(part=="assembly") {
    color([0.2,0.65,0.65]) adapter();
    color([0.8,0.8,0.82]) collector();
    color([0.8,0.8,0.82]) lid();
    color([0.85,0.6,0.25]) transition();
    color([0.6,0.6,0.65]) outlet();
    color([0.7,0.75,0.79])
        tube(203,201,collector_l+transition_l+outlet_l+joint_gap,spool_l);
    clamp_reference(collector_l+transition_l+outlet_l);
    clamp_reference(fan_y-joint_gap);
    blower_reference();
    color([0.7,0.75,0.79]) tube(203,201,fan_y+fan_l+joint_gap,exhaust_l);
